...
var Hook = exports.Hook = function (options) {
  ...
  //
  // By default, hook IS NOT silent.
  // If silent, the hook will not output any STDOUT
  //
  this.silent = this.silent || options.silent || false;

  //
  // By default, Standard IN ( STDIN ) is enabled
  //
  this.ignoreSTDIN  = this.ignoreSTDIN  || options.ignoreSTDIN  || false;
...
Hook.prototype.emit = function (event, data, callback, sender) {
  ...
  //
  self.log(this, event, data, sender);
  ...
  //
  return EventEmitter.prototype.emit.apply(this, [event, data, callback, sender]);
};
...
// #### @hooks {String} The hooks which should be killed
// #### @callback {function} Continuation to respond to when complete
// Kills a hook, Frees up cpu power.
//
Hook.prototype.kill = function(hooks, callback) {
  ...
  //
  // If not options have been specified, kill the current hook
  //
  ...
  //
  // If we have been passed in a String, assume it's the name of a local hook
  //
  if (typeof hooks === 'string') {
    //
    // Check if any children hooks exist with this name
    //
    if (self.children[hooks] && self.children[hooks].monitor) {
      self.stop();
      self.children[hooks].monitor.stop();
      callback(null);
    } else {
      callback(new Error('No child hook exists with name: ' + hooks));
    }
  }

  //
  // TODO: If we have been passed in options hash
  //
  ...
  self.toIPs(self['hook-host'], function onResolve (err, hosts) {
    ...
    self.server = dnode(function (client, conn) {
      ...
      this.message = function (event, data, callback, sender) {
        ...
        //
        // This is the point-of-entry for all client-generated events,
        // so append the sender parameter here. For security purposes
        // we always override it so it can't be spoofed.
        //
        sender = null;
        if(!this.remote) {
          sender = self._senderInfo(client);
        }

        self.emit(event, data, callback, sender);
      };
      ...
      //
      self.onAny(function (data, callback, sender) {
        ...
        // TODO: make use of sender parameter
