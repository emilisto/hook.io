...
var listen = function(options, callback) {
  ...
  Hook.prototype.listen.call(this, options, function(err) {
    ...
    // you're not going to need host and domain. host is very special and
    // domain ... i've never seen cross-domain browsing in action, or wide
    // area mDNS ... just drop the options [agnat]
    ...
    var startedCb = function(error, service) {
      // Hey! What happend to 'FIXME: check for errors'? ... you should probably
      // do this. Maybe like this: when you get an error report it (to
      // a logging hook or something?), back off for half a second and then try 
      // to start a new ad. Also, emit the service object because it contains 
      // information on what is actually advertised. [agnat]
...
var connect = function(options, callback) {
  ...
  // like i said: rename 'info' to 'service' and remove the flags
  // argument. [agnat]
  serviceUp = function(info, flags) {
    ...
    // like above: just expose the name option [agnat]
  ...
  // TODO: mv info service && rm flags [agnat]
  ...
  //
  // Hm, that's right. mdns delivers all pending events even after
  // stop() has been called. Although, no IO is happening after stop().
  // I can understand that this is unexpected. Open an issue, if you
  // like.
  // However, I'm still not getting what you are trying to achieve.
  // I guess it has something to do with hook.io's "startup procedure"?
  // Could you explain it a bit more? [agnat]
